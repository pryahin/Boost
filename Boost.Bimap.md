...
Библиотека [Boost.Bimap](http://www.boost.org/libs/bimap) основана на Boost.MultiIndex, и предоставляет контейнер, который может быть использован будучи не определенным сначала. Этот контейнер подобен `std::map`, но поддерживает просмотр значений с обеих сторон. Boost.Bimap позволяет создавать отображения, где любая сторона может быть ключом, в зависимости от того, как вы получаете доступ к отображениям. Когда вы получаете доступ к левой стороне как к ключу, правая сторона - значение, и наоборот.
...
```
[Пример 13.4](#example134) использует контейнер типа `boost::bimaps::multiset_of`, который определен в `boost/bimap/multiset_of.hpp`. Он работает как `boost::bimaps::set_of`, за исключением того, что ключи могут быть не уникальными. [Пример 13.4](#example134) успешно покажет **2** при поиске животных с четырьмя лапами.

Поскольку `boost::bimaps::set_of` используется по умолчанию для контейнеров типа `boost::bimap`, заголовочный файл `boost/bimap/set_of.hpp` может не включаться явно. Однако, при использовании других типов контейнеров, должны быть включены соответствующие им заголовочные файлы.

В дополнение к классам, показанным выше, Boost.Bimap предоставляет следующие типы контейнеров: `boost::bimaps::unordered_set_of`, `boost::bimaps::unordered_multiset_of`, `boost::bimaps::list_of`, `boost::bimaps::vector_of`, и `boost::bimaps::unconstrained_set_of`. За исключением `boost::bimaps::unconstrained_set_of`, все другие контейнерные типы работают точно так же, как их "коллеги" из стандартной библиотеки.

<a name="example135"></a>
### Пример 13.5. Отключение одной стороны с использованием `boost::bimaps::unconstrained_set_of`
```c++
#include <boost/bimap.hpp>
...
```
`boost::bimaps::unconstrained_set_of` может использоваться, чтобы исключить одну сторону boost::bimap. В [Примере 13.5](#example135), `boost::bimap` ведет себя как `std::map`. Вы не можете получить доступ к **right**, чтобы искать животных по количеству лап.

[Пример 13.5](#example135) иллюстрирует другую причину, почему может иметь смысл предпочитать `boost::bimap` нежели `std::map`. Boost.Bimap доступны функции из Boost.MultiIndex, так как он основан на Boost.MultiIndex. [Пример 13.5](#example135) изменяет ключ, используя modify_key() – то, что невозможно с `std::map`.
